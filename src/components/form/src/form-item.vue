<template>
  <div class="s-form-item" :class="{
    ...
    <label :for="prop" class="s-form-item__label" :style="labelStyle" v-if="label">
      {{label + form.labelSuffix}}
    </label>
    <div class="s-form-item__content" :style="contentStyle">
      <slot></slot>
      <transition name="s-zoom-in-top">
        <div class="s-form-item__error" v-if="validateState === 'error' && showMessage && form.showMessage">{{validateMessage}}</div>
      </transition>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      getRules() {
        ...
        var ssr = [].concat(selfRuels || formRules || []);
